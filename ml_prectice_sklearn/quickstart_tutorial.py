...
#     self.flatten = nn.Flatten()
#     self.linear1 = nn.Linear(16 * 28 * 28, 512)
...
print(f"Using {device} device")
class MyNeuralNetwork_1(nn.Module):
  def __init__(self):
    super(MyNeuralNetwork_1, self).__init__()
    self.conv2d = nn.Conv2d(1, 16, kernel_size=3, padding=1)
    self.conv_transpose2d = nn.ConvTranspose2d(16, 1, kernel_size=3, stride=2, padding=1, output_padding=1)
    self.flatten = nn.Flatten()
...
model = MyNeuralNetwork_1().to(device)
print(model)
